...
generations = 2000  # 迭代次数
chrom_length = 22 # 染色体长度
pc = 0.6  # 交配概率
pm = 0.01  # 变异概率
genetic_population = []  # 种群的基因编码（二进制）
population = []  # 种群对应的十进制数值,并标准化范围到[-1, 2]
fitness = []  # 适应度
fitness_mean = []
...
def chrom_decoding():
    ...
    for i in range(population_size):
        ...
        population.append((value * 3 / (2 ** (chrom_length) - 1))-1)
...
def calculate_fitness():
    ...
    fitness.clear()
    for i in range(population_size):
        function_value = population[i]* math.cos(4 * population[i]*3.141592654) + population[i]**2
